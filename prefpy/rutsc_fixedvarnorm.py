...
# by Azari, Parkes, & Xia
# This file implements the fixed-variance,
...
def estimate_theta(n, m, pi, N, epsilon, max_iters, true_theta):
    ...
    Parameters:
        ...
        pi:         agents' orderings over alternatives (n by m matrix)
    ...
    theta_t = np.random.normal(loc=0, scale=1, size=(m))
        ...
        for j in range(m):
            ...
            for i in range(n):
                ...
                if next_best == None:
                    ...
                    rval = x_k[i][next_best] - theta_j
                else: # next_best > j
                    rval = x_N[k][i][next_best] - theta_j

                if next_worst == None:
                    lval = stats.norm.a # -inf
                elif next_worst < j:
                    lval = x_k[i][next_worst] - theta_j
                else: # next_worst > j
                    lval = x_N[k][i][next_worst] - theta_j

                distr =  stats.truncnorm(a=np.array([lval]), b=np.array([rval]), loc=theta_j, scale=1)
                sample = distr.rvs() # sample the truncated Normal distribution
                x_k[i][j] = sample

        x_N.append(x_k)
        
    #return np.average(np.array(x_N[349:]), axis=0) # discard first 350 samples
    return np.average(x_N, axis=0) # average ALL samples
...
def mstep(n, m, S_t):
    ...
    """
    theta_t1 = np.average(S_t, axis=0)
    return theta_t1
...
def test1():
    ...
    150 agents
    4 alternatives
    1000 iterations in round-robin Gibbs sampler
    500 maximum iterations of MC-EM
    1*10**-12 epsilon convergence condition
    """
    n = 150
    m = 4
    zeta = np.random.normal(loc=0, scale=1, size=(n, m)) # agents' subjective noise
    actual_theta = np.random.normal(loc=0, scale=1, size=(m))
    X = actual_theta + zeta
    pi = []
    for i in range(n):
