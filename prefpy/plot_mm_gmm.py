...
def plot_mse_ktau_data(mse_results, ktau_results, output_img_filename=None):
    ...
    plt.title("MSE")
    plt.xlabel("n (votes)")
    mse_line, = plt.plot(mse_results.T[0], mse_results.T[1], "bs", label="MM")
    ktau_line, = plt.plot(mse_results.T[0], mse_results.T[2], "g^", label="GMM-F")
    plt.subplot(122)
    plt.title("Kendall Correlation")
    plt.xlabel("n (votes)")
    plt.plot(ktau_results.T[0], ktau_results.T[1], "bs", label="MM")
    plt.plot(ktau_results.T[0], ktau_results.T[2], "g^", label="GMM-F")
    fig.legend([mse_line, ktau_line], ["MM", "GMM-F"], loc="center right")
